...
final class RedundantInternalAccessibilityMarker: SourceGraphMutator {
    ...
    private func shouldSkipMarking(_ decl: Declaration) -> Bool {
        ...
        // Skip declarations referenced by #Preview macro expansions
        if isReferencedByPreviewMacro(decl) {
            return true
        }
    ...
    /// Checks if a declaration is referenced by a `#Preview` macro expansion.
    ///
    /// When `retainSwiftUIPreviews` is enabled, `#Preview` macros are retained. However, the
    /// implicit declarations generated by these macros cannot have access modifiers. Types
    /// referenced only by `#Preview` must remain `internal` — making them `fileprivate` or
